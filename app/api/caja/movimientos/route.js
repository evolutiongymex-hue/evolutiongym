...
export async function GET() {
  try {
    ...
    const [movimientosRes, pagosRes] = await Promise.all([
      sheets.spreadsheets.values.get({
        spreadsheetId: SHEET_ID,
        range: "CAJA_MOVIMIENTOS!A:H",
      }),
      sheets.spreadsheets.values.get({
        spreadsheetId: SHEET_ID,
        range: "PAGOS!A:K",
    ...
    const pagos = (pagosRes.data.values ?? []).slice(1);
    const hoy = new Date().toLocaleDateString("en-CA", {
      timeZone: "America/Mexico_City",
    });

    // Ingresos en efectivo del día desde PAGOS
    const ingresosEfectivoHoy = pagos
      .filter((p) => p[3] === hoy && (p[5] ?? "").toLowerCase() === "efectivo")
      .reduce((s, p) => s + (parseFloat(p[4]) || 0), 0);
    ...
    // Fondos agregados hoy
    const fondosHoy = movimientosHoy
      .filter((m) => m.tipo === "fondo")
      .reduce((s, m) => s + m.monto, 0);

    // Saldo real en caja = último saldo registrado + ingresos efectivo hoy - retiros hoy + fondos hoy
    // El saldo_nuevo ya incluye movimientos anteriores
    // Solo necesitamos agregar ingresos efectivo que no están en CAJA_MOVIMIENTOS
    const saldoEnCaja = saldoBase + ingresosEfectivoHoy;
    ...
    return NextResponse.json(
      { error: "Error al obtener caja: " + err.message },
      { status: 500 }
    );
  }
}
...
export async function POST(request) {
  try {
    ...
    const saldoActual = saldoBase + ingresosEfectivoHoy;
